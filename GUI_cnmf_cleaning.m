...
function varargout = GUI_cnmf_cleaning(varargin)
% GUI_CNMF_CLEANING MATLAB code for GUI_cnmf_cleaning.fig
%      GUI_CNMF_CLEANING, by itself, creates a new GUI_CNMF_CLEANING or raises the existing
%      singleton*.
%
%      H = GUI_CNMF_CLEANING returns the handle to a new GUI_CNMF_CLEANING or the handle to
%      the existing singleton*.
%
%      GUI_CNMF_CLEANING('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_CNMF_CLEANING.M with the give0n input arguments.
%
%      GUI_CNMF_CLEANING('Property','Value',...) creates a new GUI_CNMF_CLEANING or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_cnmf_cleaning_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_cnmf_cleaning_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @GUI_cnmf_cleaning_OpeningFcn, ...
    'gui_OutputFcn',  @GUI_cnmf_cleaning_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before GUI_cnmf_cleaning is made visible.
function GUI_cnmf_cleaning_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_cnmf_cleaning (see VARARGIN)

% Choose default command line output for GUI_cnmf_cleaning
...
% UIWAIT makes GUI_cnmf_cleaning wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_cnmf_cleaning_OutputFcn(hObject, eventdata, handles)
...
guiName = 'GUI_cnmf_cleaning';  % Change this to your actual GUI function name
